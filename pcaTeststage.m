function [score,l] = pcaTeststage(P_test,P_train,training_number,rank,C,c,pn)
%        --- Calculate PCA algorithm classification accuracy---
%
%	Input:         'P_train'         - training set after projection
%
...
M = zeros(r,c);
for i = 1:c
    M(:,i) =sum( P_train(:,(i - 1) * n_j + 1 : (i - 1) * n_j + n_j),2) / n_j;
end
H = P_test;
k=(pn - n_j) * c;
l = zeros(k,c);
